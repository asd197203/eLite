...
- (BOOL)tableExists:(NSString*)tableName {
    ...
    CXZResultSet *rs = [self executeQuery:@"select [sql] from sqlite_master where [type] = 'table' and lower(name) = ?", tableName];
...
- (BOOL)executeUpdate:(NSString*)sql
 withArgumentsInArray:(NSArray*)arrayArgs
    ...
    if (_shouldCacheStatements && !cachedStmt) {
        cachedStmt = [[CXZStatement alloc] init];
...
- (CXZResultSet *)executeQuery:(NSString*)sql, ... {
    va_list args;
    va_start(args, sql);
...
- (CXZResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
    return [self executeQuery:sql withArgumentsInArray:nil orDictionary:nil orVAList:args];
}

- (CXZResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
...
- (CXZResultSet *)executeQuery:(NSString *)sql
    ...
    int rc                  = 0x00;
    sqlite3_stmt *pStmt     = 0x00;
    CXZStatement *myStmt    = 0x00;
    CXZResultSet *rs        = 0x00;
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(CXZExecuteStatementsBlock)block {
    ...
    rc = sqlite3_exec(self.sqliteHandler,
                      sql.UTF8String,
                      block ? CXZDataBaseExecuteBulkSQLCallback : nil,
                      (__bridge void *)(block),
                      &errmsg);
...
- (void)closeOpenResultSet {
    ...
    for (NSValue *rsInWrappedInATastyValueMeal in _openResultSet) {
        CXZResultSet *rs = (CXZResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(CXZResultSet *)resultSet {
...
- (CXZStatement *)cachedStatementForQuery:(NSString *)query {

    NSMutableSet *statements = _cachedStatements[query];

    return [[statements objectsPassingTest:^BOOL(CXZStatement *statement, BOOL *stop) {
...
- (void)setCachedStatement:(CXZStatement *)statement forQuery:(NSString *)query {
...
- (NSError *)errorWithMessage:(NSString *)message {
    return [NSError errorWithDomain:@"CXZDatabase"
...
- (void)setMaxBusyRetryTimeInterval:(NSTimeInterval)maxBusyRetryTimeInterval {
    ...
    if (maxBusyRetryTimeInterval) {
        // 数据库繁忙时 尝试一波gank
        sqlite3_busy_handler(_db, &CXZDataBaseBusyHandler, (__bridge void *)(self));
    }
    else {
...
static int CXZDataBaseBusyHandler(void *_self, int count) {
    CXZDataBase *self = (__bridge CXZDataBase *)_self;
...
static int CXZDataBaseExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
#pragma mark-  CXZStatement

@implementation CXZStatement
...
#pragma mark-  CXZResultSet

@interface CXZResultSet () {
    @package
    CXZDataBase *_parentDB;
}

@end

@implementation CXZResultSet

@dynamic resultDictionary;

@synthesize columnNameToIndexMap = _columnNameToIndexMap;

+ (instancetype)resultSetWithStatement:(CXZStatement *)statement usingParentDatabase:(CXZDataBase *)aDB {

    CXZResultSet *rs = [[CXZResultSet alloc] init];
...
- (void)setParentDB:(CXZDataBase *)parentDB {
    _parentDB = parentDB;
}
