...
@interface CDChatManager () <AVIMClientDelegate, AVIMSignatureDataSource,UIAlertViewDelegate>
...
- (void)sendWelcomeMessageToOther:(NSString *)other text:(NSString *)text block:(AVBooleanResultBlock)block {
    [self fetchConversationWithOtherId:other callback:^(AVIMConversation *conversation, NSError *error) {
        if (error) {
            ...
            AVIMTextMessage *textMessage = nil;
            if ([text containsString:@"我已将你删除####"]) {
                NSMutableDictionary *attributes = [[NSMutableDictionary alloc]init];
                [attributes setObject:@"delete" forKey:@"delete"];
                textMessage = [AVIMTextMessage messageWithText:text attributes:attributes];
            }
            else
            {
               textMessage = [AVIMTextMessage messageWithText:text attributes:nil];
            }
...
- (void)client:(AVIMClient *)client didOfflineWithError:(NSError *)error {
    if ([error code] == 4111) {
        ...
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"提示" message:@"检测到您已在其他设备登录，请重新登录" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alert show];
    }
}
