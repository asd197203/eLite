...
- (void)configPeerName:(BOOL)displayPeerName atMessage:(id<XHMessageModel>)message {
    ...
    if (displayPeerName) {
        ...
        [GetUserInfoReq do:^(id req) {
            ...
            re.friendid =[message sender];
        } Res:^(id res) {
            GetUserInfoRes *resq = res;
            if (resq.code==0) {
                if ([resq.data[@"isfriend"] integerValue]==1) {
                    if (![resq.data[@"remark"] isEqualToString:@""]) {
                        self.peerNameLabel.text = resq.data[@"remark"];
                    }
                    else
                    {
                      self.peerNameLabel.text = resq.data[@"name"];
                    }
                }
                else
                {
...
- (void)configAvatorWithMessage:(id<XHMessageModel>)message {
    ...
    [GetUserInfoReq do:^(id req) {
        ...
        if (resq.code==0) {
            if (resq.data[@"photo"]) {
                self.avatorButton.layer.cornerRadius = 4;
                self.avatorButton.clipsToBounds = YES;
                [self.avatorButton setImage:message.avator forState:UIControlStateNormal];
                 [self.avatorButton setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",serverUrl,resq.data[@"photo"]]] placeholer:[UIImage imageNamed:@"lcim_conversation_placeholder_avator"]];
...
//                 [self.contentView sendSubviewToBack:statusView];
...
- (void)layoutSubviews {
    ...
    CGFloat bubbleX = 0.0f;
    if ([self bubbleMessageType] == XHBubbleMessageTypeReceiving) {
//        if(self.messageBubbleView.message.attr[@"userid"])
//        {
