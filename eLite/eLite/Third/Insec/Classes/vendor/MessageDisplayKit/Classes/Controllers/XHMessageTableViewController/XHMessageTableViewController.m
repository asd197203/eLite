...
@property (nonatomic,strong) GetFriendModel*model;//用于保存名片的信息
/**
 ...
 *  @param dic 目标名片信息
 */
- (void)didSendMessageWithVistingCard:(NSDictionary *)dic;
/**
 *  根据文本开始发送文本消息
 *
 *  @param text 目标文本
...
- (void)initilzer {
    ...
    if ([XBUserDefault sharedInstance].resModel.messageBgImage) {
        UIImageView *backImageView=[[UIImageView alloc]initWithFrame:self.view.bounds];
        backImageView.image = [XBUserDefault sharedInstance].resModel.messageBgImage;
...
- (void)didSendMessageWithPhoto:(UIImage *)photo {
    ...
    if ([self.delegate respondsToSelector:@selector(didSendPhoto:fromSender:onDate:VistingUser:)]) {
        if (_model==nil) {
            [self.delegate didSendPhoto:photo fromSender:self.messageSender onDate:[NSDate date] VistingUser:nil];
        }
        else
        {
          [self.delegate didSendPhoto:photo fromSender:self.messageSender onDate:[NSDate date] VistingUser:_model];
        }
    }
    _model=nil;
}
...
- (void)didSelecteShareMenuItem:(XHShareMenuItem *)shareMenuItem atIndex:(NSInteger)index {
    ...
    switch (index) {
        ...
        case 2: {
            ...
            RecordVideoView *recordView = [[RecordVideoView alloc]init];
            recordView.frame = CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height);
...
//            [self.view addSubview:view];
        ...
        case 3: {
            WEAKSELF
            XHDisplayLocationViewController *locationVC = [[XHDisplayLocationViewController alloc]init];
            locationVC.GetGeolocationsCompledBlock = ^(NSArray *placemarks,CLLocation*loc)
            {
                CLPlacemark *placemark = [placemarks lastObject];
                if (placemark) {
                    NSDictionary *addressDictionary = placemark.addressDictionary;
                    NSArray *formattedAddressLines = [addressDictionary valueForKey:@"FormattedAddressLines"];
                    NSLog(@"placemark.location---%@",loc);
                    NSString *geoLocations = [formattedAddressLines lastObject];
                    if (geoLocations) {
                        [weakSelf didSendGeolocationsMessageWithGeolocaltions:geoLocations location:loc];
                    }
                }
            };
            locationVC.isLocationMessage = NO;
...
//            }];
        ...
        case 4: {
            XBFriendListToVistingCardVC*vistingCardVC = [[XBFriendListToVistingCardVC alloc]init];
            vistingCardVC.delegate = self;
            UINavigationController *nav = [[UINavigationController alloc]initWithRootViewController:vistingCardVC];
...
- (void)didFinishRecordingAtURL:(NSURL *)outputFileURL error:(NSError *)error
{
    [self didSendMessageWithVideoConverPhoto:nil videoPath:[NSString stringWithFormat:@"%@",outputFileURL]];
    
}
- (void)selectFriend:(GetFriendModel *)friend
{
    _model = friend;
    UIImageView *imageView = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"名片背景"]];
    imageView.size = CGSizeMake(240, 100);
    UILabel *label = [[UILabel alloc]init];
    label.frame = CGRectMake(20, 5, 80, 30);
    label.text = @"个人名片";
    label.font = [UIFont systemFontOfSize:14];
    [imageView addSubview:label];
    UIImageView *headImage = [[UIImageView alloc]initWithFrame:CGRectMake(20, 35, 50, 50)];
    [headImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",serverUrl,_model.photo]]];
    [imageView addSubview:headImage];
    UILabel *namelabel = [[UILabel alloc]init];
    namelabel.font = [UIFont systemFontOfSize:14];
    namelabel.frame =CGRectMake(80, 40, [Util getSizeWithText:_model.name font:[UIFont systemFontOfSize:14]].width, 30);
    namelabel.text = _model.name;
    [imageView addSubview:namelabel];
    UIImage *image = [self imageCache:imageView];
    [self didSendMessageWithPhoto:image];
    
}
- (UIImage*)imageCache:(UIImageView*)imageView {
