...
- (void)layoutSubviews {
    ...
    switch (currentType) {
        ...
        {
            if (self.message.bubbleMessageType ==XHBubbleMessageTypeReceiving) {
                if (self.message.attr[@"userid"]) {
                    CGRect photoImageViewFrame = CGRectMake(bubbleFrame.origin.x, 0, 240, 100);
            ...
            {
                if (self.message.attr[@"userid"]) {
                    CGRect photoImageViewFrame = CGRectMake(bubbleFrame.origin.x-120, 0, 240, 100);
                    self.bubblePhotoImageView.frame = photoImageViewFrame;
                }
                else
                {
