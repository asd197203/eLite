...
@property(nonatomic,strong)AVIMConversation *con;
@end
...
- (void)viewDidLoad {
    ...
    if (_con.type== CDConversationTypeSingle) {
...
- (void)goChatGroupDetail:(id)sender {
    CDConvDetailVC *DetailVC = [[CDConvDetailVC alloc] init];
...
//        [self.navigationController pushViewController:userInfoVC animated:YES];
        AVUser *user =[[CDCacheManager manager] lookupUser:msg.clientId];
        XBUserInfoVC *InfoVC = [[XBUserInfoVC alloc]init];
        InfoVC.user = user;
        InfoVC.userID = user.username;
